...
/**
 * Eden's canonical Nostr account: the same npub the catalog (kind 30402) is
 * published from and that the footer's "Nostr" social link points to. This is
 * the account visitors follow.
 */
...
/**
 * A "Follow Me" button that follows Eden's Nostr account by adding it to the
...
export function FollowMeButton({
  ...
  const follow = async () => {
    ...
    try {
      ...
      toast({
        title: 'Following ✓',
        description: "You're now following Eden on Nostr.",
      });
    } catch (error) {
      toast({
        title: 'Could not follow',
  ...
  return (
    <div className={cn('flex flex-col items-center gap-2', className)}>
      <Button
        ...
        aria-label={alreadyFollowing ? 'Following Eden on Nostr' : 'Follow Eden on Nostr'}
      ...
      >
        ...
        {alreadyFollowing ? 'Following' : 'Follow Me'}
      </Button>
