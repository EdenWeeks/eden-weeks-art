...
import { buildGammaFromNip15 } from '@/lib/nip15Migration';
...
export function useProductAdmin() {
  ...
  /**
   * Transfer a NIP-15 (kind 30018) listing to NIP-99/Gamma (kind 30402). The
   * Gamma event reuses the NIP-15 id as its d tag, so the storefront's unified
   * query stops showing the NIP-15 copy the moment this publishes.
   */
  const migrateNip15Product = useMutation({
    mutationFn: (vars: { data: Nip15ProductData; event: NostrEvent }) =>
      publish(buildGammaFromNip15(vars.data, vars.event)),
    onSuccess: invalidateProducts,
  });
